...
def register_NAVEnv(
    ...
    register(
        id="NAVEnv-v0",
        entry_point='spinup.envs.NAVEnv:env',
        kwargs=dict(
            is_training=True
        ),
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    def thunk_plus():
        ...
        if 'env_name' in kwargs:
            ...
            from spinup.run import CUSTOMIZING_ENV_NAMES
            if env_name in CUSTOMIZING_ENV_NAMES:
                ### register custumizing env
                register_NAVEnv(
                        env_id=env_name, 
                        is_training=True
                )
            print(f"[INFO] {env_name} environment has been registered.")
